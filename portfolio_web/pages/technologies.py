...
from portfolio_web.components.styles import Size, Color, TextColor, Font
...
def technologies():
    return rx.vstack(
        rx.heading(
            "Technologies",
            style={"font_family": Font.TITLES.value},
            font_size=Size.VERY_BIG.value,
            padding_top=Size.BIG.value,
            padding_bottom=Size.SMALL.value,
            color=TextColor.SECONDARY.value,
        ),
        rx.text("Backend & Database:", color=TextColor.SECONDARY.value),
        rx.grid(
            rx.foreach(ForeachState.backend, logos),
            columns=rx.breakpoints(initial="3", xs="4", sm="5", md="8", lg="9"),
        ),
        rx.text("Frontend:", color=TextColor.SECONDARY.value),
        rx.grid(
            rx.foreach(ForeachState.frontend, logos),
            columns=rx.breakpoints(initial="3", xs="4", sm="5", md="8", lg="9"),
        ),
        rx.text("Tools & Methodologies:", color=TextColor.SECONDARY.value),
        rx.grid(
