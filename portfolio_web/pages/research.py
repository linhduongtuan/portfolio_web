...
from portfolio_web.components.styles import Color, TextColor, Size
...
def research_item(proj: Research) -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.vstack(
            rx.heading(
                "Outputs", size=Size.LIGHT_BIG.value, color=TextColor.PRIMARY.value
            ),
...
def research_view() -> rx.Component:
    ...
    return rx.box(
        rx.vstack(
            rx.heading(
                "PROJECTS & RESEARCH",
                size=Size.BIG.value,  # Make it larger
                color=TextColor.PRIMARY.value,
