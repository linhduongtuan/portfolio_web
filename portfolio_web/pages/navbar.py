...
from portfolio_web.components.styles import Color, Size, TextColor
...
def dark_mode_toggle() -> rx.Component:
    return rx.flex(
        rx.color_mode_cond(
            dark=rx.icon(tag="moon", size=20), light=rx.icon(tag="sun", size=20)
        ),
...
def is_current_page(page_route: str) -> rx.Var:
    ...
    return rx.State.router.page.path == page_route
...
def navbar() -> rx.Component:
    return rx.box(
        rx.hstack(
            ...
            rx.spacer(),
            highlighted_button_menu("HOME", "HOME", Links.HOME, False),
            highlighted_button_menu(
                "layout-dashboard", "RESEARCH", Links.RESEARCH, False
            ),
            highlighted_button_menu("book", "PUBLICATION", Links.PUBLICATIONS, False),
            highlighted_button_menu("pen", "BLOG", Links.BLOG, False),
            highlighted_button_menu("user", "ABOUT", Links.ABOUT, False),
            rx.menu.root(
                rx.menu.trigger(
